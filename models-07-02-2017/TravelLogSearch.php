...
//     	]);
    ...
    public function details($params)
    {
    	...
    	$date = $params['date']; 
    	$query = TravelLog::find()->select(['id, location_name, activity, distance_travelled, start_time, updated_date as end_time,order_number,submission_mode as mode,TIMESTAMPDIFF(MINUTE,start_time,updated_date) / 60 as time'])
								    	 ->from('plan_cards')
								    	 ->where(['status' => 'submitted'])
								    	 ->andWhere(['assign_to' => $ff['assign_to']])
								    	 ->andWhere(['DATE_FORMAT(updated_date, "%Y-%m-%d")' => $date]);
    	$query2 = UserTravellog::find()->select(['id, location_name, type as activity, distance_travelled, start_time, date_time as end_time,order_number,mode,TIMESTAMPDIFF(MINUTE,start_time,date_time) / 60 as time'])
