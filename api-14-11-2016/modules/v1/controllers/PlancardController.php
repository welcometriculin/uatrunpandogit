...
// 		$products_favourites = FavProducts::favProductsInsert($params, $user_id);
	...
	public function actionChcard()
	{
		...
		$locations = \app\models\CampaignCardActivities::locationList();
		$command = yii::$app->db->createCommand('UPDATE plan_cards SET status = "submitted", submission_mode = "online", lat_position ='.$lat.', long_position ='.$long.', location_name = "'.$location.'", distance_travelled = "'.$distance.'" , updated_date = NOW(), start_time = "'.$start_time.'" WHERE id="'.$card_id.'"');
		$command->execute();
