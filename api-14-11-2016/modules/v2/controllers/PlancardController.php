...
use app\api\modules\v1\controllers\SyncdbController;
...
//plancard creation 
	public function actionCreatecard()
	{
		...
		$villages = \app\models\Villages::villageData($date,$user_id);
		$channel_partners = \app\models\ChannelPartners::partnersdata($date,$user_id);
...
// 		$products_favourites = FavProducts::favProductsInsert($params, $user_id);
	...
	public function actionSubmit()
	{
		...
		//user travel log type checking end
		 $result= \app\models\PlanCards::dataSubmit($params,$id);
		 $locations = \app\models\CampaignCardActivities::locationList();
	...
	{
			$list = \app\models\CampaignCardActivities::locationList();
			if(empty($list)){
	...
	public function actionAllcards()
	{
    	...
    	$model = $this->modelClass;
     	...
     	$sum_pendingcards = \app\models\PlanCards::countCards($user_id);
     	$week_pendingcards = \app\models\PlanCards::wcountCards($user_id);
	...
	{
		$params =Yii::$app->getRequest()->getBodyParams();
		$data = \app\models\MasterProducts::products($params['hit_timestamp']);
	...
	public function actionChcard()
	{
		...
		if(!empty($chct_info)) {
			$chct_info = \app\models\ChannelCardTrackingInfo::trackinginfo($chct_info,$params);
		}
		$chc_activities = \app\models\ChannelCardActivities::channelcardinfo($params);
		$locations = \app\models\CampaignCardActivities::locationList();
		$command = yii::$app->db->createCommand('UPDATE plan_cards SET status = "submitted", submission_mode = "online", lat_position ='.$lat.', long_position ='.$long.', location_name = "'.$location.'", distance_travelled = "'.$distance.'" , updated_date = NOW(), start_time = "'.$start_time.'", order_number = "'.$orderValue.'" WHERE id="'.$card_id.'"');
		$command->execute();
	...
	public function actionTravellog()
	{
		...
		$travel_log_info = \app\models\PlanCards::travelDetails($params);
	...
	public function actionTravellogstartstop()
	{
		...
		$web_model->params = json_encode($params);
		$web_model->save(false);
		$travel_log_start = \app\models\UserTravellog::travellogstart($params);
