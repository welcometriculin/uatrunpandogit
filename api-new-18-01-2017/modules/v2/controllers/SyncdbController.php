...
use app\api\modules\v2\models\PlanCards;
use app\api\modules\v2\models\WebService;
use app\api\modules\v2\models\Crops;
use app\api\modules\v2\models\Products;
use app\api\modules\v2\models\Villages;
use app\api\modules\v2\models\CampaignCardActivities;
use app\api\modules\v2\models\MasterProducts;
use app\api\modules\v2\models\ChannelPartners;
use app\api\modules\v2\models\Activity;
use app\api\modules\v2\models\SubActivity;
use app\api\modules\v2\models\FavCrops;
use app\api\modules\v2\models\FavProducts;
use yii\db\Expression;
use app\api\modules\v2\models\UserTravellog;
use app\api\modules\v2\models\ChannelCardActivities;
use app\api\modules\v2\models\FavVillages;
use app\api\modules\v2\models\FavChannelpartners;
use app\api\modules\v2\models\ChannelCardTrackingInfo;
		...
		if (array_key_exists('user_travellog', $params)) {
			...
			if(!empty($user_travellog)) {
				foreach ($user_travellog as $travellog ) {
					...
					$user_travel_log[] = array($user_id, $travellog['latitude_position'],$travellog['longitude_position'], $location_name, $distance, new Expression('NOW()'), new Expression('NOW()'), $travellog['type'],$tr_timestamp,'offline',$travellog['order_number']);
				}
		...
		if (array_key_exists('campaian_activity', $params) && count($params['campaian_activity']) > 0) {
			...
			if(count($campaign_activity) >0) {
				foreach($campaign_activity as $activity) {
					...
					$planCardDist[$activity['data']['plan_card_id']] = ['lat' => $lat, 'long' => $long, 'distance' => $distance, 'location' => $location, 'start_time' => $start_time,'order_number' => $ord_value];
			...
			if(!empty($params['channel_activity']['data'])) {
				...
				if(count($channel_activity) >0 ){
					...
					foreach($channel_activity as $ch_activity) {
						...
						$channelplanCardDist[$ch_activity['plan_card_id']] = ['lat' => $lat2, 'long' => $long2, 'distance' => $channel_distance, 'location' => $channel_location, 'start_time' => $start_time2, 'order_number' => $OrderValue];
						$channel_array[] = array($guid3,$card_id3, $ch_activity['feedback'], $user_id, new Expression('NOW()'), $user_id, new Expression('NOW()'), $user_id, $chact_timestamp,'offline');
						$products = $ch_activity['products'];
						if (!empty($products)) {
							foreach($products as $product) {
								$ch_products[] = array($guid4,$card_id3,$product['product_id'],$product['product_unit'],$product['liquidation_status'],$product['demand_volume'],$product['season_progress'],$product['collection_value_four'],$product['collection_value_five'],$user_id,new Expression('NOW()'),$user_id,new Expression('NOW()'));
							}
						}
					}
