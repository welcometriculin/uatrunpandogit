...
class Villages extends  Kg
{
    ...
    public function attributeLabels()
    {
        return [
            'id' => 'ID',
            'guid' => 'Guid',
            'village_name' => (count($label_names_display = LabelNames::labelNamesDisplay()) > 0 ? ucfirst($label_names_display['village_label']) :'Village'),
            'comp_id' => 'Comp ID',
...
//     	echo '<pre>';print_r($res_arr);exit;
    ...
    //Total Villages
    public static function allVillages($fieldForce)
    {
    
    	$villQuery =  (new \yii\db\Query())
    	->select('vm.village_name')
    	->from('plan_cards p')
    	->leftJoin('villages_master vm','vm.village_id = p.village_id')
    	->where(['p.assign_to' => $fieldForce,'p.status' => 'submitted'])
    	->groupBy('vm.village_name')
    	->orderBy('vm.village_name')
    	->column();
    	/* $fieldForceS = implode(',',$fieldForce);
    	 $villQuerys = 'SELECT p.village_id, vm.village_id, vm.village_name FROM plan_cards p
    	 LEFT JOIN villages_master vm ON vm.village_id = p.village_id
    	 WHERE (p.assign_to IN ('.$fieldForceS.') )
    	 AND (p.status="submitted")
    	 GROUP BY vm.village_name ORDER BY vm.village_name';
    	$villQueryResult = Yii::$app->db->createCommand($villQuerys)->queryAll(); */
    	return $villQuery;
