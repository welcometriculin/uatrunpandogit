...
class Users extends Kg
{
	...
	public $designation_name; // list view in profile page
    /**
     * @inheritdoc
     */
    ...
    public function rules()
    {
        return [
        	...
        	[['first_name', 'employee_number','email_address', 'phone_number', 'roleid', 'reporting_user_role', 'reporting_user_id', 'password', 'created_date', 'created_by', 'updated_date', 'updated_by','designation_id'], 'required'],
        	...
        	[['first_name', 'head_quarters', 'area_of_operatoin'], 'match', 'pattern' => '/^[a-zA-Z0-9\s]+$/'],
        	//[['status'], 'string'],
            [['state', 'district', 'head_quarters', 'area_of_operatoin','designation_id'], 'safe'],
        	['email_address', 'email'],
    ...
    public function attributeLabels()
    {
        return [
            ...
            'area_of_operatoin' => 'Area of Operation',
            'status' => 'Status',
...
/*   	$fflist =  Users::find()->select('id,first_name,email_address')->where(['input_company_id' =>$comp_id,'roleid' => 4 ])->asArray()->all();
  ...
  public static function dashboardUsers($res)
   {
  	...
  	$repquery->select('u.id,u.first_name,r.role_name,d.designation_name')
			  	->from('users u')
			  	->innerJoin('roles r' ,'r.id = u.roleid')
			  	->leftJoin('designations d' ,'d.designation_id = u.designation_id')
			  	->where(['u.id' => $res])
  	...
  	if(!empty($queryresp)) {
  		$listArray = ArrayHelper::map($queryresp,'id',function ($element) {
  			if($element['designation_name'] == '') {
  				$element['designation_name'] = 0;
  			}
  			return ucfirst($element['first_name']) . ' - '. ucfirst($element['designation_name']).'-'.ucfirst($element['role_name']);
  		});
  	}
  	return $listArray;
  }
  ...
  public static function usersList()
  {
  	...
  	$rep = Users::getRecoursive($userId,true);
  	$usersQuery = 'select sum(if((u.roleid = 4),1,0)) as app_users,sum(if((u.roleid = 3 or u.roleid = 2),1,0)) as web_users
					from users u where u.reporting_user_id = '.$userId;
  	$usersQueryResult = Yii::$app->db->createCommand($usersQuery)->queryOne();
  	$query = TravelLog::find()->select('p.assign_to')
  	->from('plan_cards p')
  	->innerJoin('users u', 'u.id = p.assign_to')
  	->where(['p.status' => 'submitted','p.assign_to' => $rep])
  	->groupBy(['p.assign_to']);
  	$query2 = TravelLog::find()->select(" ut.user_id AS assign_to")
  	...
  	->groupBy(['assign_to'])
  	->column();
  	return $userslistData = array('app_users' => count(array_unique($union)),'web_users' => $usersQuery['web_users']);
  	 
  }
  public static function usersCount()
  {
  	$webUsers = Users::userlistRecoursive(Yii::$app->user->identity->id,3,true);
  	if(!is_array($webUsers)) {
  		$webUsers = array();
  	}
  	$appUsersCount = Users::find()->select('id')->where(['roleid' =>4,'input_company_id' => Yii::$app->user->identity->input_company_id,'is_deleted' => Users::is_deleted])->count();
  	// $appusersQuery = Users::find()->select('id')->where(['id' => $appUsers])->andWhere(['is_login' => 1])->count();
  	return $noOfusers = ['web_users' => count($webUsers),'app_users' => $appUsersCount];
  }
  public static function getManagers($pid,$role_id,$recoursive=false)
  {
  	//echo 'rec'.$pid;exit;
  	$companyIds = Users::find()->select('roleid,id')->where(['reporting_user_id'=>$pid])->asArray()->all();
  	if(empty($companyIds)) {
  	...
  	foreach($companyIds as $data){
  		...
  		if($recoursive) {
  			$data['childs'] = Users::getManagers($cid, true);
  		}
  		$r[] = $data;
  	}
  	return $Reportids;
  }
  public static function userRecoursive($pid2,$recoursive=false)
  {
  	//echo 'rec'.$pid;exit;
  	$companyIdsList = Users::find()->select('roleid,id')->where(['reporting_user_id'=>$pid2])->asArray()->all();
  	if(empty($companyIdsList)) {
  		return $pid2;
  	}
  	$r = array();
  	global $userids;
  	foreach($companyIdsList as $dataInfo){
  		$cid = $dataInfo['id'];
  		if($dataInfo['roleid'] == 4) {
  			$userids[] = $dataInfo['id'];
  		}
  		if($recoursive) {
  			$dataInfo['childs'] = Users::userRecoursive($cid, true);
  		}
  		$r[] = $dataInfo;
  	}
  	return $userids;
  }
  public static function userlistRecoursive($logID,$roleId,$recoursive=false)
  {
  	//echo 'rec'.$pid;exit;
  	$companyIdsListD = Users::find()->select('roleid,id,is_deleted')->where(['reporting_user_id'=>$logID])->asArray()->all();
  	if(empty($companyIdsListD)) {
  		return $logID;
  	}
  	$rdd = array();
  	global $useridsData;
  	foreach($companyIdsListD as $dataInfoD){
  		$cid = $dataInfoD['id'];
  		if($dataInfoD['roleid'] == $roleId && $dataInfoD['is_deleted'] == Users::is_deleted) {
  			$useridsData[] = $dataInfoD['id'];
  		}
  		if($recoursive) {
  			$dataInfoD['childs'] = Users::userlistRecoursive($cid, $roleId, true);
  		}
  		$rdd[] = $dataInfoD;
  	}
  	return $useridsData;
  }
  public static function usersDetailsdata($usersListData)
  {
  	$usersQueryData = array();
  	$usersQueryData = (new \yii\db\Query())
  	->select(['u.first_name,u.employee_number,uu.first_name as RM,u.email_address,u.phone_number,r.role_name,IF(u.head_quarters is null or u.head_quarters = "", "N/A", u.head_quarters) as head_quarters,IF(u.status = "active", "Yes", "NO") as active,IF(u.is_login = "1", "Yes", "NO") as is_login'])
  	->from('users u')
  	->leftJoin('users uu','u.reporting_user_id = uu.id')
  	->leftJoin('roles r','u.roleid = r.id')
  	->where(['u.is_deleted'=> 0,'u.id'=> $usersListData])
  	->orderBy('u.is_login DESC,u.first_name ASC')
  	->groupBy('u.id')->all();
  	return $usersQueryData;
  }
  public static function getUpdatedby($userId)
  {
  	return $dataInfo = Users::findOne($userId);
  }
