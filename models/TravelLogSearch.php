...
class TravelLogSearch extends TravelLog
{
    ...
    public function search($params)
    {
    	...
    	if(empty($params)) {
    		...
    		if(!isset($params['TravelLogSearch'])) {
    			$id = Yii::$app->user->identity->id;
    		} elseif($params['TravelLogSearch']['assign_to'] == '') {
				$id = Yii::$app->user->identity->id;
			} else {
				$id = $params['TravelLogSearch']['assign_to'];
			}
    			
    	}
    	$rep = Users::getRecoursive($id,true);
    	//echo '<pre>';print_r($rep);exit;
...
//     	echo '<pre>';print_r(var_dump($union->prepare(Yii::$app->db->queryBuilder)->createCommand()->rawSql));exit;
...
//     	]);
    ...
    public function details($params)
    {
    	...
    	$date = base64_decode(str_pad(strtr($params['date'], '-_', '+/'), strlen($params['date']) % 4, '=', STR_PAD_RIGHT));
//     	$date = $params['date']; 
    	$userName = (new \yii\db\Query())
    	->select('first_name')
    	->from('users')
    	->where(['id' => $ff['assign_to']])
    	->one();
    	$query = TravelLog::find()->select(['id, location_name, activity, distance_travelled, start_time, updated_date as end_time,order_number,submission_mode as mode,TIMESTAMPDIFF(SECOND,updated_date,start_time) as time'])
								    	 ->from('plan_cards')
								    	 ->where(['status' => 'submitted'])
								    	 ->andWhere(['assign_to' => $ff['assign_to']])
								    	 ->andWhere(['DATE_FORMAT(updated_date, "%Y-%m-%d")' => $date]);
    	$query2 = UserTravellog::find()->select(['id, location_name, type as activity, distance_travelled, start_time, date_time as end_time,order_number,mode,TIMESTAMPDIFF(SECOND,date_time,start_time) as time'])
    	...
    	$dataProvider = new ActiveDataProvider([
     	...
     	return ['dataProvider' => $dataProvider,'ff_name'=>$userName['first_name'], 'distance_travelled' => $distance_travelled['distance_travelled']];
        }
}
