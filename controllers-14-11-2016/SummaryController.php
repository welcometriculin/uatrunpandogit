...
// 		->execute();
		...
		if (!empty($queryresp)) {
			...
			$this->managersPlanSummary($ff, $duration);
 			//$this->updateMainPlanSummaryTable($current_year, $current_month, $duration);
		} else {
			echo 'Not Saved';
		}
	}
	private function managersPlanSummary($users, $duration) 
	{
		if(isset($_GET['debug'])){
			echo "Fetching managers<br/>";
		}
		...
		if($mgCnt > 0) {
			...
			//	echo $roleId;echo '<br />';
				$query = "SELECT  $mangerId as user_id, sum(total) as total,2016 as year,sum(rejected) as rejected, sum(accepted) as accepted, sum(bc) as bc, sum(bnc) as bnc, 
...
// 				$res = Yii::$app->db->createCommand($sql)->execute();
			...
			if (count($ids) > 0){
				$this->managersPlanSummary($ids, $duration); # recursive function
			}
