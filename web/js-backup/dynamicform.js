...
/* dynamic form save button click start*/
 ...
 $('#dynamic-form-save').on('beforeSubmit', function(){
	...
	if ($("input[name='FormBuilder[step_2_field1_require]']:checked").val() == 1) {
		...
		/*var sorted_arr = step2_field1_unique_values_arr.slice().sort(); // You can define the comparing function here. 
		...
		for (var i = 0; i < step2_field1_unique_values_arr.length - 1; i++) {
		    if (sorted_arr[i + 1] == sorted_arr[i]) {
		    	step2_field1_unique_values_arr_results.push(sorted_arr[i]);
		    }
		}
		if (step2_field1_unique_values_arr_results.length > 0) {
			$(".field-formbuilder-step_2_field1_field_value").find('.help-error').html('Duplicate values are enterd.');
		} else {
			$(".field-formbuilder-step_2_field1_field_value").find('.help-error').html('');
		}*/
	...
	if (step_2_field1_field_boxes_values.length != step_2_field1_lenth || step_3_field1_field_boxes_values.length != step_3_field1_lenth || step_3_field2_field_boxes_values.length != step_3_field2_lenth || step_3_field3_field_boxes_values.length != step_3_field3_lenth || step_3_field4_field_boxes_values.length != step_3_field4_lenth || step_3_field5_field_boxes_values.length != step_3_field5_lenth || step_4_field1_field_boxes_values.length != step_4_field1_lenth || fields_count == 1 || minimum_steps === false) {
		return false;
    } else {
    	return true;
    }
