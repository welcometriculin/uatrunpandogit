...
use app\api\modules\v3\models\Roles;
...
class Users extends Kg
{
	...
	public $company_status; //used in LoginAction
    /**
     * @inheritdoc
     */
  ...
  /* web service for forgot password end */
   ...
   public static function userProfileEdit($params)
   {
	...
	$old_phone_check = Users::find()->select(['phone_number'])->Where(['!=', 'id', $user_id])->andWhere(['phone_number'=> $phone_number])->count();
	if ($old_phone_check >= 1) {
		return 0;
	} else {
		if ($params['photo'] != '') {
