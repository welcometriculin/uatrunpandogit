...
/**
 * This is the model class for table "village_wise_yearly_activity_summary".
 *
 * @property string $id
 ...
 */
class VillageWiseYearlyActivitySummary extends \yii\db\ActiveRecord
{
    /**
     * @inheritdoc
     */
    ...
    {
        return 'village_wise_yearly_activity_summary';
    }
    ...
    {
        return [
            [['user_id', 'village_name', 'demo', 'fgm', 'mc', 'fhv', 'total', 'month', 'year','village_id'], 'required'],
            [['user_id', 'demo', 'fgm', 'mc', 'fhv', 'total', 'month','village_id'], 'integer'],
            [['year'], 'safe'],
    ...
    //for web dashboard performance
    public static function villagePerformance($user_id, $time)
    {
    	$current_year = date('Y');
    	$current_month = date('m');
    		//$model = new VillageWiseYearlyActivitySummary();
    	$query = (new \yii\db\Query());
    		$village_wise_activity_summary = $query->select('mv.village_name,va.village_id,SUM(va.demo) as demo, SUM(va.fgm) as fgm, SUM(va.mc) as mc, SUM(va.fhv) as fhv, SUM(va.total) as total')
    							->from('village_wise_yearly_activity_summary as va')
    							->innerJoin('villages_master mv','va.village_id = mv.village_id')
    							->where(['va.user_id' => $user_id, 'va.year' => $current_year])
