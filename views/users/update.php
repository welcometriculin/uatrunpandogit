...
<div class="panel-body panel-bg">
	<div class="form row col-md-12 mt20">
		...
		<div class="form-group  col-xs-12 col-sm-6 col-md-4">
    	<?= $form->field($model, 'email_address')->textInput(['maxlength' => true,'placeholder'=>'Enter Email Address','readonly'=>false]) ?>
		</div>
		<div class="form-group  col-xs-12 col-sm-6 col-md-2">
...
//for dependency users start
	dependency_result = false;
		...
		setTimeout(
			  function() 
			  {
			    rep =  $('#users-reporting_user_id').val();
			  }, 2000);
		$('#users-reporting_user_id').change(function(){
			$('.save').click(function(){
		...
		if(rep != reporting_user_id) {
			var flash = "Due to dependency you can't assign other role";
			if (manager_id == '') {
				dependency_result = false;
			} else {
				$.ajax({
