...
<div class="villages-update">
  ...
  <div class="clearfix">
	<?= $form->field($model, 'managefield',['template' => "{label}\n<div class='col-xs-12 col-sm-6 col-md-6 col-lg-4 '>{input}\n{hint}\n{error}</div>"])->dropDownList($mmlist,['prompt' => 'Select Manager','disabled'=>'disabled'])->label('Manager Name',['class'=>'col-xs-12 col-sm-5 col-md-3 col-lg-3 control-label'])  ?></div>

      <div class="clearfix">
	<?= $form->field($model, 'user_id',['template' => "{label}\n<div class='col-xs-12 col-sm-6 col-md-6 col-lg-4'>{input}\n{hint}\n{error}</div>"])->dropDownList($fflist,['prompt' => 'Select Field Force Name','disabled'=>'disabled'])->label('Field Force Name',['class'=>'col-xs-12 col-sm-5 col-md-3 col-lg-3  control-label'])  ?></div>
          <div class="clearfix">
    <?= $form->field($model, 'village_name',['template' => "{label}\n<div class='col-xs-12 col-sm-6 col-md-6 col-lg-4'>{input}\n{hint}\n{error}</div>"])->textInput(['maxlength' => true])->label((count($label_names_display) > 0 ? ucfirst($label_names_display['village_label']) :'Village'),['class'=>'col-xs-12 col-sm-5 col-md-3 col-lg-3 control-label'])  ?>
      </div>
