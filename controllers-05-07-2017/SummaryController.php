...
class SummaryController extends Controller
{
	...
	public function actionPlanyearsummary()
	{
		...
		FROM `plan_cards` where is_deleted = 0 and YEAR(`updated_date`) = $current_year and Month(`updated_date`)<= $current_month group by `assign_to`";
		...
		if (!empty($queryresp)) {
			...
			foreach ($data as $key => $val){
				...
				WHERE user_id in ([[ids]]) and year = $current_year";
				...
				$this->runplansRecoursive($val,$params);
			}
		} else {
			echo 'Not Saved';
		}
	...
	public function runplansRecoursive($arr,$params) {
		$ids = array();
		if(isset($arr['childs'])) {
			foreach($arr['childs'] as $c) {
				$ids[] = $c['id'];
				$this->runplansRecoursive($c,$params);
			}
			if(is_array($arr['childs']) && sizeof($arr['childs'])>0) {
				$query = str_replace(array('[[id]]','[[ids]]'),array($arr['id'],implode(',',$ids)),$params['query']);
				$table = $params['table'];
				$whereCond = $params['whereCond'];
				$updateFields = $params['updateFields'];
				$this->insertPlansummary($query, $table, $whereCond, $updateFields);
			}
		}
		return;
	}
	...
	private function insertPlansummary($query, $table, $whereCond, $updateFields)
	{
		$result = Yii::$app->db->createCommand($query)->queryAll();
		if($result[0]['total'] != '') {
			foreach ($result as $rec) {
	...
	public function actionPlanmonthsummary()
	{
		...
		sum(if((`status`!='submitted' and `plan_approval_status`!='Rejected' and created_by!=assign_to and plan_type='planned'),1,0)) as `anc_planned`,
		sum(if((`status`!='submitted' and `plan_approval_status`!='Rejected' and created_by!=assign_to and plan_type='adhoc'),1,0)) as `anc_adhoc`
		FROM `plan_cards` where is_deleted = 0 and YEAR(`updated_date`) = $current_year and Month(`updated_date`) = $current_month and day(`updated_date`)<= $month_end_date group by `assign_to`";
		$queryresp = Yii::$app->db->createCommand($plan_summary)->queryAll();
		//print_r($queryresp);exit;
		$reportingManagers = array();
		$ff = array();
