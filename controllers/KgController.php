	...
	public function beforeAction($action)
	{
		...
		if ($loginsession) {
			if (Yii::$app->session['userSessionTimeout'] < time()) {
				...
				return $this->redirect(['site/expired'])->send();
			} else {
				Yii::$app->session->set('userSessionTimeout', time() + Yii::$app->params['sessionTimeoutSeconds']);
			}
			$company_status = array();
			$company_status = InputCompanies::find()->select('status')->where(['id' => Yii::$app->user->identity->input_company_id])->asArray()->one();
		...
		if (Yii::$app->user->isGuest  && ($controllerid =='inputcompanies') && ($actionid == 'create')) {
			...
			if (Yii::$app->user->identity->roleid == $icadmin_role_id) {
				...
				if ($menuscountusers > 0 && $company_status['status'] == 'active' && (($controllerid == 'plancard' && $actionid == 'index') || ($controllerid == 'plancard' && $actionid == 'create') || ($controllerid == 'plancard' && $actionid == 'update') || ($controllerid == 'plancard' && $actionid == 'history') || ($controllerid == 'travellog' && $actionid == 'index') || ($controllerid == 'travellog' && $actionid == 'view') || ($controllerid == 'channelpartners' && $actionid == 'index'))) {
					return true;
				} 
			}
		} elseif ($loginsession == '') {
		...
		//print_r($queryresp);
		//exit;
		if (Yii::$app->user->identity->id != '' && $company_status['status'] == 'active' && ($controllerid == $queryresp['controller']) &&  ($actionid == $queryresp['action']) && Yii::$app->user->identity->status == 'active' && Yii::$app->user->identity->is_blocked == 0 && Yii::$app->user->identity->is_deleted == 0) {
			return true;
		} else {
			Yii::$app->session->destroy();
			return $this->redirect(['site/expired'])->send();
