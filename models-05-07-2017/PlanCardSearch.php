...
class PlanCardSearch extends PlanCards
{
	...
	public function search($params)
	{
		...
		if(empty($params)) {
			...
			if(!isset($params['PlanCardSearch'])) {
				$id = Yii::$app->user->identity->id;
			} elseif($params['PlanCardSearch']['assign_to'] == '') {
				$id = Yii::$app->user->identity->id;
			} else {
				$id = $params['PlanCardSearch']['assign_to'];
			}
				
		}
		//echo $id;exit;
		$resultUsers = Users::userRecoursive($id,true);
		//     	echo '<pre>';print_r($resultUsers);exit;
		if (($action == 'history') && $controller == 'plancard') {
			$condition = ['=','p.status','submitted'];
			$orderby = ' p.updated_date desc,p.order_number desc';
		} else {
			$condition = ['!=','p.status','submitted'];
