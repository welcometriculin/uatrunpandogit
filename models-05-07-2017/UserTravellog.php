...
class UserTravellog extends \yii\db\ActiveRecord
{
    ...
    //for travelog start stop web service
    public static function travellogstart($params)
    {
    	...
    	$model->mode = 'online';
    	$start_time = self::travelstart_time();
    ...
    //for travelog start stop web service end
    //for user travelog start_time web service start
    public static function travellogstart_time() 
    {
    	 
    	$user_id = Yii::$app->user->identity->id;
    	$current_date = date('Y-m-d');
	    $usertravellog = \app\models\UserTravellog::find()->select('date_time')
...
//  	    echo '<pre>';print_r($usertravellog);exit;
	  /*  if(empty($usertravellog)) {
	    ...
	    $plancards_updatetime = \app\models\PlanCards::find()->select('updated_date')
    ...
    public static function travelstart_time()
    {
    	$user_id = Yii::$app->user->identity->id;
    	$current_date = date('Y-m-d');
    	$usertravellog = \app\models\UserTravellog::find()->select('date_time')
