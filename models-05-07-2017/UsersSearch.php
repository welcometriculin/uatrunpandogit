...
class UsersSearch extends Users
{
    ...
    public function search($params)
    {
    	...
    	if ($user_role_id == $kgadmin_role_id) {
			...
			$query = Users::find()->select('u.*,d.designation_name')
			->from('users u')
			->leftJoin('designations d','u.designation_id = d.designation_id')
			->where(['u.input_company_id' => $ic_id, 'u.is_deleted' => 0])
			->andWhere(['!=','u.roleid',$user_role_id])
			->orderBy('u.id DESC');
       	} else if($user_role_id == $manager_role_id) 	{
       		$query = Users::find()->select('u.*,d.designation_name')
       		->from('users u')
       		->leftJoin('designations d','u.designation_id = d.designation_id')
       		->where(['u.input_company_id' => $ic_id, 'u.is_deleted' => 0,'reporting_user_id' => $reporting_user_id])
       		->orderBy('u.id DESC');
        ...
        if (!empty($params) && !empty($params['UsersSearch'])) {
        	if (!is_numeric($params['UsersSearch']['free_text_search'])) {
        		$query->andFilterWhere(['or',
        				...
        				['like', 'd.designation_name', trim($this->free_text_search)]
        				]);
        	} else {
        		$query->having(['or',
        				['like', 'employee_number', trim($this->free_text_search)],
