...
//     					$template .= '     Your account is activated successfully. <br><br> You can login into your account by using your credentials.<br><br>Regards <br>Kisan Gates Support Team';
    ...
    public function actionProfile()
    {
    	...
    	if (isset($_POST['bulk'])) {
    		...
    		if (strip_tags($rowDataheader[0][1]) != 'Employee Name' || strip_tags($rowDataheader[0][2]) != 'Employee Id' || strip_tags($rowDataheader[0][3]) != 'Phone Number' || strip_tags($rowDataheader[0][4]) != 'Email Id' || $highestColumn != 'K') {
    			$check = 0;
    		...
    		if ($check == 1) {
	    		for ($row = 2; $row <= $highestRow; $row++) {
	    			...
	    			$finalData[$row]['reporting_user_id'] = trim($rowData[0][6]);
	    			$finalData[$row]['state'] = trim($rowData[0][7]);
	    			$finalData[$row]['district'] = trim($rowData[0][8]);
	    			$finalData[$row]['head_quarters'] = trim($rowData[0][9]);
	    			$finalData[$row]['area_of_operatoin'] = trim($rowData[0][10]);
	    			$finalData[$row]['input_company_id'] = Yii::$app->user->identity->input_company_id;
	//     			$password = rand(1000, 10000);
	//     			$finalData[$row]['password'] = md5($password);
	    			$password = 'Goodluck@123';
	    		...
	    		if (!empty($finalData)) {
		    		...
		    		for($i = 2;$i < $length;$i++){
		    			...
		    			if (!preg_match("/^[a-zA-Z\s]+$/", $finalData[$i]['first_name'])) {
		    				$check = 0;
