...
class SummaryController extends Controller
{
	...
	private function managersPlanSummary($users, $duration,$fieldForceData)
	{
		if(isset($_GET['debug'])){
			echo "Fetching managers<br/>";
		}
		$current_year = date("Y");
		$adminIds = $reportIDS = array();
		$managersInfo = Users::getReportingManagers($users);
		$mgCnt = count($managersInfo);
		if($mgCnt > 0) {
			if(isset($_GET['debug'])){
			...
			if(!empty($managersInfo)) {
				foreach($managersInfo as $manager) {
			$reporteesArray = explode(',',$manager['reportees']);
			foreach($reporteesArray as $manID) {
			if(array_key_exists($manID,$fieldForceData)) {
			$managerData[$manager['id']][$manID] = $fieldForceData[$manID];
			}
			...
			$managerKeys = array_keys($managerData);
			if(!empty($managerKeys)) {
			foreach($managerKeys as $man) {
			$queryCh = Yii::$app->db->createCommand('select count(id) as count,group_concat(id) as manager from users where reporting_user_id = "'.$man.'"')->queryOne();
			$managerArray = explode(',',$queryCh['manager']);
			foreach($managerArray as $m) {
			if(array_key_exists($m,$managerData)) {
			array_push($managerData[$man],$managerData[$m]);
			unset($managerData[$m]);
			}
			}
			...
			for($i = 0; $i<= count(array_keys($managerData)); $i++) {
			$managerData = $this->managerData($managerData);
			...
			for ($j=0; $j < $mgCnt; $j++) {
				...
				$mangerId = $managersInfo[$j]['id'];
				$reportees = $managersInfo[$j]['reportees'];
				$roleChecking = "SELECT roleid FROM users WHERE id = $mangerId";
				$roleCheckingRole = Yii::$app->db->createCommand($roleChecking)->queryOne();
				$roleId = $roleCheckingRole['roleid'];
				//	echo $roleId;echo '<br />';
				$query = "SELECT  $mangerId as user_id, sum(total) as total,$current_year as year,sum(rejected) as rejected, sum(accepted) as accepted, sum(bc) as bc, sum(bnc) as bnc,
				sum(ac) as ac, sum(anc) as anc, sum(bc_planned) as bc_planned, sum(bc_adhoc) as bc_adhoc, sum(ac_planned) as ac_planned, sum(ac_adhoc) as ac_adhoc,
				sum(bnc_planned) as bnc_planned, sum(bnc_adhoc) as bnc_adhoc, sum(anc_planned) as anc_planned, sum(anc_adhoc) as anc_adhoc
				FROM plan_wise_yearly_summary
				WHERE user_id in ($reportees)";
				$table = "plan_wise_yearly_summary";
				$whereCond = array('user_id','year');
				$updateFields = array('total','rejected', 'accepted', 'bc', 'bnc',
						'ac', 'anc', 'bc_planned', 'bc_adhoc', 'ac_planned', 'ac_adhoc', 'bnc_planned', 'bnc_adhoc', 'anc_planned', 'anc_adhoc');
				$field = $updateFields[0];
				$this->insertAndUpdateOnDupe($query, $table, $whereCond, $updateFields);
				// 				$res = Yii::$app->db->createCommand($sql)->execute();
			}
			echo '<pre>';print_r($ids);//exit;
// 			$this->managerData($ids,$current_year);
			
			 if (count($ids) > 0){
			$this->managersPlanSummary($ids, $duration,$managerData); # recursive function
			} 

			$reportees = array();$reportees_array = array();
		}


	}
	
	private function managerData($ids,$current_year)
	{
		if(!empty($ids)) {
			foreach($ids as $managerID) {
				$roleid = Yii::$app->db->createCommand('SELECT roleid FROM users WHERE id = "'.$managerID.'"')->queryScalar();
				if($roleid != Roles::ICADMIN) {
					if($managerID != 0) {
					$managersList = $this->lowlevelManagers($managerID);
					 echo '<pre> managers list';print_r($managersList);
					/*$reportIDS[] = $getReportID;
					$managerQuery = "SELECT  $getReportID as user_id, sum(total) as total,$current_year as year,sum(rejected) as rejected, sum(accepted) as accepted, sum(bc) as bc, sum(bnc) as bnc,
					sum(ac) as ac, sum(anc) as anc, sum(bc_planned) as bc_planned, sum(bc_adhoc) as bc_adhoc, sum(ac_planned) as ac_planned, sum(ac_adhoc) as ac_adhoc,
					sum(bnc_planned) as bnc_planned, sum(bnc_adhoc) as bnc_adhoc, sum(anc_planned) as anc_planned, sum(anc_adhoc) as anc_adhoc
					FROM plan_wise_yearly_summary
					WHERE user_id in ($managerID)";
					$table = "plan_wise_yearly_summary";
					$whereCond = array('user_id','year');
					$updateFields = array('total','rejected', 'accepted', 'bc', 'bnc',
							'ac', 'anc', 'bc_planned', 'bc_adhoc', 'ac_planned', 'ac_adhoc', 'bnc_planned', 'bnc_adhoc', 'anc_planned', 'anc_adhoc');
					$this->insertAndUpdateOnDupe($managerQuery, $table, $whereCond, $updateFields); */

					}
				} else {
					$icAdminarray[] = $managerID; 
				}
			}
			if(!empty($reportIDS)) {
				$this->managerData($reportIDS,$current_year);
			}

		}
	}
	private function lowlevelManagers($managerID = 726)
	{
		$filedForce = Roles::FIELDFORCE;
		$getManagers = Yii::$app->db->createCommand('SELECT group_concat(id) as id FROM users WHERE reporting_user_id = "'.$managerID.'" and roleid !='.$filedForce)->queryAll(); 
		  if($getManagers[0]['id'] != '') { 
		$managerArray = explode(',',$getManagers[0]['id']);
		if(!empty($managerArray)) {
			foreach($managerArray as $manId) {
				if($manId > 0) {
					return $manArray[] = $manId;
					$sa = 'dss';
					$this->lowlevelManagers($manId);
				} else {
					return $sa;
				}
			}
	...
	/* private function managerData($originalData)
	 {
	// 		echo '<pre>original data';print_r(json_encode($originalData));
	$adminIds = $managerKeys = array();
	// 		echo '<pre>adsdadasd';print_r(array_keys($originalData));exit;
	$managersInfo = Users::getReportingManagers(array_keys($originalData));
	echo '<pre>adsdadasd';print_r($managersInfo);
	$mgCnt = count($managersInfo);
	if(!empty($managersInfo)) {
	foreach($managersInfo as $manager) {
	$reporteesArray = explode(',',$manager['reportees']);
	foreach($reporteesArray as $manID) {
	if(array_key_exists($manID,$originalData)) {
	$originalData[$manager['id']][$manID] = $originalData[$manID];
	unset($originalData[$manID]);
	}
	...
	// 			echo '<pre> 1st level managers';print_r(array_keys($originalData));
	$managerKeys = array_keys($originalData);
	$managerKeys1 = $managerKeys;
	if(!empty($managerKeys)) {
	foreach($managerKeys as $man) {

	$queryCh = Yii::$app->db->createCommand('select count(id) as count,group_concat(id) as manager from users where reporting_user_id = "'.$man.'" and roleid != 4')->queryOne();
	$managerArray = explode(',',$queryCh['manager']);
	// 					echo '<pre>';print_r($managerArray);exit;
	foreach($managerArray as $m) {
	if(in_array($m,$managerKeys1)) {
	if(!empty($originalData[$man])) {
	array_push($originalData[$man],$originalData[$m]);
	unset($originalData[$m]);
	}
	} else {
	...
	if (!empty($managersInfo)){
	$this->managerData($originalData); # recursive function
	}
	// 			echo '<pre>original data';print_r(json_encode($originalData));exit;
	return $originalData;
	} */
	...
	public function actionAdminplans() {
		$roleid = 2;
		$admins = Users::find()->select('id')->where(['roleid' => $roleid,'status' =>'active'])->asArray()->column();
		if(!empty($admins)) {
			foreach($admins as $adminId) {
				$reportees_array_final = '';
				$reportingUsers = "SELECT id from users where reporting_user_id = $adminId";
				$reportees_array = Yii::$app->db->createCommand($reportingUsers)->queryColumn();
				$reportees_array_final = implode(',',$reportees_array);
				if(!empty($reportees_array)) {
				$query2 = "SELECT  $adminId as user_id, sum(total) as total,year,sum(rejected) as rejected, sum(accepted) as accepted, sum(bc) as bc, sum(bnc) as bnc,
				sum(ac) as ac, sum(anc) as anc, sum(bc_planned) as bc_planned, sum(bc_adhoc) as bc_adhoc, sum(ac_planned) as ac_planned, sum(ac_adhoc) as ac_adhoc,
				sum(bnc_planned) as bnc_planned, sum(bnc_adhoc) as bnc_adhoc, sum(anc_planned) as anc_planned, sum(anc_adhoc) as anc_adhoc
				FROM plan_wise_yearly_summary
				WHERE user_id in ($reportees_array_final)";
				echo '<br />';
				$table = "plan_wise_yearly_summary";
				$whereCond = array('user_id','year');
				$updateFields = array('total','rejected', 'accepted', 'bc', 'bnc',
						'ac', 'anc', 'bc_planned', 'bc_adhoc', 'ac_planned', 'ac_adhoc', 'bnc_planned', 'bnc_adhoc', 'anc_planned', 'anc_adhoc');
				$field = $updateFields[0];
				$this->insertAndUpdateOnDupe($query2, $table, $whereCond, $updateFields,$field);
